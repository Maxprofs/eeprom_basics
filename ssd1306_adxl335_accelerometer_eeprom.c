...
int main(void)
{
    ...
    init_io();                                  // Initialize Status LEDs
    init_interrupts();                          // Initialize Pin Change Interrupt(PCINT) 
    i2c_init();                                 // Initialize I2C interface

    ret = i2c_start(DevSSD1306+I2C_WRITE);       // set device address and write mode
    if (ret) {
        i2c_stop();                             // Unenable issue start condition to slave- possibly no device found
        PORTB= (1<<PB1);                            // Drive LED to show error */
    }
    ...
    uint8_t byteOfData;
    uint8_t writeThisByte;
    char byteOfDataAsString[8];

    writeThisByte = 0x01;
    eeprom_update_byte((uint8_t*)46, writeThisByte);
    byteOfData = eeprom_read_byte((uint8_t*)46);
    clearBuffer(buffer);
    
    itoa(byteOfData, byteOfDataAsString, 10);
    lcd_draw_string(0, 0, byteOfDataAsString, buffer);
